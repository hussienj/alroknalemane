...
import { LogOut, Home, Bell, Shield, BookOpen, Calendar, ClipboardCheck, ShieldBan, Gamepad2, Swords, ChevronsLeft, ChevronsRight, Award, Trophy, ListChecks, BookText, Settings as SettingsIcon, RefreshCw, MessageCircle, ClipboardList, Compass } from 'lucide-react';
import { ensureDefaultSportsAndArtSubjects } from '../../constants.ts';
...
import MoodCheckInModal from '../../components/student/MoodCheckInModal.tsx';
import ClassAdvisorGuidanceView from '../../components/student/ClassAdvisorGuidanceView.tsx';
...
type StudentView = 'dashboard' | 'admin_messages' | 'teacher_messages' | 'schedule' | 'monthly_results' | 'behavior_log' | 'xo_game' | 'challenges' | 'honor_board' | 'my_homework' | 'my_progress' | 'hall_of_fame' | 'homework_submission' | 'educational_encyclopedia' | 'school_forum' | 'student_form' | 'advisor_guidance';
...
export default function StudentApp({ currentUser, onLogout }: StudentAppProps) {
    ...
    useEffect(() => {
        ...
        try {
            const localPhoto = localStorage.getItem(storageKeyPhoto);
            if (localPhoto) setStudentFormPhoto(localPhoto);
        } catch (e) {}

        const principalId = currentUser.principalId || 'principal_al_hamza';

        const locksRef = db.ref(`form_locks/${principalId}`);
        const locksCallback = (snapshot: any) => {
            if (snapshot.exists()) {
        ...
        db.ref(`student_saved_forms/${principalId}/${studentKey}`).get().then(snap => {
            if (snap.exists() && snap.val().studentPhoto) {
                const photo = snap.val().studentPhoto;
                setStudentFormPhoto(photo);
                try { localStorage.setItem(storageKeyPhoto, photo); } catch (e) {}
            } else {
                db.ref(`student_submissions/${principalId}/${studentKey}`).get().then(subSnap => {
                    if (subSnap.exists() && subSnap.val().studentPhoto) {
                        const photo = subSnap.val().studentPhoto;
                        setStudentFormPhoto(photo);
                        try { localStorage.setItem(storageKeyPhoto, photo); } catch (e) {}
                    }
                });
        ...
        return () => {
            locksRef.off('value', locksCallback);
        };
    }, [currentUser]);
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                ...
                if (classesData) {
                    const allClassesList: ClassData[] = Object.values(classesData).map((c: any) => ({
                        ...c,
                        subjects: ensureDefaultSportsAndArtSubjects(c.subjects || [])
                    }));
    ...
    const handlePhotoUpdate = async (photoBlob: Blob) => {
        if (isFormLocked) {
            alert("تم قفل تغيير الصورة الشخصية وتعديل الاستمارة من قبل إدارة المدرسة.");
            throw new Error("Photo update locked");
        }
        const principalId = currentUser.principalId || 'principal_al_hamza';
        const studentKey = currentUser.id || currentUser.code || 'default_student';
        const storageKeyPhoto = `student_form_photo_${studentKey}`;
        ...
        let photoURL = '';
        try {
            const photoId = studentData?.id || studentKey;
            const photoRef = storage.ref(`student_photos/${principalId}/${photoId}.jpg`);
        ...
        setStudentFormPhoto(photoURL);
        try { localStorage.setItem(storageKeyPhoto, photoURL); } catch (e) {}

        // Save to Firebase form drafts
        db.ref(`student_saved_forms/${principalId}/${studentKey}`).update({
            studentPhoto: photoURL,
            updatedAt: new Date().toISOString()
        }).catch(() => {});

        if (studentData && currentUser.classId) {
            ...
            if (studentClass?.students) {
                ...
                if (studentIndex !== -1) {
                    ...
                    db.ref(path).set(photoURL).catch(() => {});
                }
            }
        }
    ...
    const renderView = () => {
        const viewLabels: Record<StudentView, string> = {
            ...
            school_forum: 'منتدى المدرسة',
            student_form: 'استمارة معلومات الطالب',
            advisor_guidance: 'توجيهات مرشد الصف'
        };
        ...
        switch(view) {
                ...
                return (
                    <div className="space-y-6">
                        ...
                        <StudentDashboard 
                            ...
                            studentFormPhoto={studentFormPhoto}
            ...
            case 'monthly_results':
                return <StudentMonthlyResults currentUser={currentUser} resultsData={monthlyResults} studentPhotoUrl={studentFormPhoto || studentData?.photoUrl} />;
            case 'behavior_log':
            ...
            case 'student_form':
                return (
                    <StudentSubmissionForm
                        submissionInfo={{
                            principalId: currentUser.principalId || 'principal_al_hamza',
                            stage: currentUser.stage || 'الاول متوسط'
                        }}
                        currentUser={currentUser}
                        onLogout={onLogout}
                    />
            ...
            case 'advisor_guidance':
                return <ClassAdvisorGuidanceView currentUser={currentUser} classes={allClasses} />;
            default:
                return (
                    <div className="space-y-6">
                        {latestGuidance && <GuidanceDisplay guidance={latestGuidance} />}
    ...
    return (
        <div className="flex h-screen bg-gray-200 relative" dir="rtl">
            {showMoodModal && <MoodCheckInModal currentUser={{ id: currentUser.id, principalId: currentUser.principalId }} onComplete={() => setShowMoodModal(false)} />}
            ...
            <div className={`bg-gray-800 text-white flex flex-col transition-all duration-300 relative ${isSidebarCollapsed ? 'w-0 p-0 border-none overflow-hidden' : 'w-64'} flex-shrink-0`}>
                ...
                <div className="flex-1 flex flex-col overflow-y-auto">
                    <nav className="px-2 py-4 space-y-2">
                        ...
                        <button onClick={() => setView('advisor_guidance')} className={`flex items-center w-full gap-3 px-4 py-2.5 rounded-lg transition-all font-bold shadow-md bg-red-600 text-yellow-300 hover:bg-red-700 hover:text-yellow-200 border border-red-500 ${isSidebarCollapsed ? 'justify-center' : ''}`} title={isSidebarCollapsed ? 'توجيهات مرشد الصف' : ''}>
                            <Compass size={20} className="text-yellow-300" />{!isSidebarCollapsed && <span>توجيهات مرشد الصف</span>}
                        </button>
                        <button onClick={() => setView('student_form')} className={`flex items-center w-full gap-3 px-4 py-2 rounded-lg transition-colors ${view === 'student_form' ? 'bg-amber-600 text-white font-bold' : 'bg-amber-600/20 text-amber-300 hover:bg-amber-600/40'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title={isSidebarCollapsed ? 'استمارة معلومات الطالب' : ''}>
                            <ClipboardList size={20} />{!isSidebarCollapsed && <span>استمارة معلومات الطالب</span>}
                        </button>
                        ...
                        <button onClick={() => setView('school_forum')} className={`flex items-center w-full gap-3 px-4 py-2 rounded-lg transition-colors ${view === 'school_forum' ? 'bg-green-600 text-white' : 'text-green-400 hover:bg-gray-700 hover:text-green-300'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title={isSidebarCollapsed ? 'منتدى المدرسة' : ''}>
                            <MessageCircle size={20} />{!isSidebarCollapsed && <span>منتدى المدرسة</span>}
                        </button>
